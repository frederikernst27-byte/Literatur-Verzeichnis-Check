...
def test_compare_to_citation_tolerates_one_year_off():
    # ±1 Jahr (Online-first vs. Druckausgabe) darf kein Fehler sein
    ...
    assert not any("Jahr" in d for d in discrepancies)


def test_compare_to_citation_flags_year_mismatch_large():
    # Mehr als 1 Jahr Unterschied soll als Abweichung gemeldet werden
    citation = Citation(number=1, raw_text="x", authors="Müller, A.", year="2020")
    candidate = Candidate("crossref", "Titel", ["A Müller"], "2009", None, None, None)
    discrepancies = compare_to_citation(citation, candidate, 100.0)
